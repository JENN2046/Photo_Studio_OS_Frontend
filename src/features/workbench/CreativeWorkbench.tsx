...
import { canWrite, isUuid, mergeCollections, SCORE_KEYS, FAILURE_CODES, evaluationScores, validateImage, canControlWorker, completeCommand } from "./domain";
...
export function CreativeWorkbench({accessToken, role, authRuntime, params}: {accessToken: string | null; role: Role | null; authRuntime: AuthRuntimeView; params: URLSearchParams}) {
  ...
  const [refresh, setRefresh] = useState(0); const [projectLoading, setProjectLoading] = useState(false); const [unitLoading, setUnitLoading] = useState(false); const loading = projectLoading || unitLoading; const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [pending, setPending] = useState(""); const [uncertain, setUncertain] = useState(false);
  const controlLock = useRef(false);
  const [sessionFailed, setSessionFailed] = useState(false); const scopeEpoch = useRef(0); const pendingLock = useRef(false);
  ...
  const clientState = useMemo(() => {
    try { return {client: createWorkbenchClient(import.meta.env.VITE_BACKEND_API_BASE_URL ?? "", window.location.origin, accessToken, writeAllowed, fetch, () => setSessionFailed(true)), error: ""}; }
    catch (error) { return {client: null, error: error instanceof Error ? error.message : "后端配置不可用。"}; }
  }, [accessToken, writeAllowed]);
  const client = clientState.client;
  const prefix = `/projects/${projectId}`;
  const units = production?.deliverables.flatMap(item => item.productionUnits) ?? [];
  const unit = units.find(item => item.id === unitId);
  const busy = !writeAllowed || Boolean(pending) || uncertain || loading;
  ...
  const reload = useCallback(async () => {
    ...
    setProduction(fresh); setAssets(refreshedAssets.items); if (snapshot) setCollections(mergeCollections({}, snapshot.collections)); setRefresh(value => value + 1);
  }, [client, projectId, prefix, unitId]);
  const run: RunAction = useCallback(async (label, action) => {
    if (!writeAllowed || pendingLock.current || uncertain) return false;
    pendingLock.current = true; setPending(label); setError(""); setNotice(""); const epoch = scopeEpoch.current;
    try {
      const result = await completeCommand(action, async () => { if (epoch === scopeEpoch.current) { setNotice(`${label}已提交。`); await reload(); } });
      if (result.error !== undefined && epoch === scopeEpoch.current) {
        if (result.error instanceof WorkbenchError && result.error.uncertain && !result.committed) setUncertain(true);
        readFailure(result.error);
        if (result.committed) setNotice(`${label}已提交，但刷新失败。请只刷新查看结果。`);
      }
      return result.committed;
    } finally { pendingLock.current = false; setPending(""); }
  }, [writeAllowed, uncertain, reload, readFailure]);
  const runControl: RunAction = useCallback(async (label, action, safeControlMode) => {
    if (!canControlWorker(writeAllowed, uncertain, safeControlMode ?? "") || controlLock.current) return false;
    controlLock.current = true; const epoch = scopeEpoch.current;
    try { await action(); if (epoch === scopeEpoch.current) { setNotice(`${label}已提交，等待 Worker 停止证据。`); setRefresh(value => value + 1); } return true; }
    catch (error) { if (epoch === scopeEpoch.current) { if (error instanceof WorkbenchError && error.uncertain) setUncertain(true); readFailure(error); } return false; }
    finally { controlLock.current = false; }
  }, [writeAllowed, uncertain, readFailure]);
  ...
  return <AppShell><main className="creative-workbench"><header className="wb-heading"><div><p className="eyebrow">PHOTO STUDIO / CREATIVE WORKBENCH</p><h1>创作工作台</h1><p>从规格到候选，再到有凭据的正式素材。</p></div><a href="#">返回命令中心</a></header>
    <div className="wb-status" aria-live="polite"><span>{writeAllowed ? "Owner 创作会话" : "只读会话"}</span>{pending && <strong>{pending}提交中…</strong>}{loading && <span>正在读取…</span>}{notice && <p>{notice}</p>}{error && <p role="alert">{error}</p>}{uncertain && <div><p>上一操作结果不确定。先刷新并检查记录；不会自动重复提交。</p><button onClick={() => { setUncertain(false); setNotice("已由你确认完成核对；新的操作须再次明确提交。"); }}>我已核对持久记录</button></div>}<button disabled={Boolean(pending) || !projectId} onClick={() => void reload().catch(readFailure)}>刷新持久记录</button></div>
    ...
    <ExecutionPanel key={`${projectId}:${unitId}`} client={client} prefix={prefix} unitId={unit.id} recipeId={recipeId} localRecipes={collections.localMediaRecipes?.items ?? []} grants={collections.grants?.items ?? []} operations={collections.operations?.items ?? []} disabled={busy} controlDisabled={!writeAllowed || loading} controlUncertain={uncertain} run={run} runControl={runControl} refresh={refresh} />
