...
Assert-ContainsLiteral -Content $smokeSource -Needle "ExpectedReadModelState" -Label "smoke script must support explicit backend 200 data-state checks"
...
Assert-ContainsLiteral -Content $mockSource -Needle "emptyBodyForPath" -Label "mock backend must support empty data response mode"
Assert-ContainsLiteral -Content $mockSource -Needle "partialBodyForPath" -Label "mock backend must support partial data response mode"
Assert-ContainsLiteral -Content $mockSource -Needle "staleBody" -Label "mock backend must support stale data response mode"
