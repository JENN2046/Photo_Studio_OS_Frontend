...
| R10 | Write actions are introduced without idempotency, conflict handling, or audit. | Critical | Medium | Blocked | No POST/PATCH/DELETE until write contract includes permission, audit, idempotency, error, and rollback behavior. |
...
| R14 | Responsive or keyboard QA coverage regresses when shared layout changes. | High | Medium | Open | Run `scripts/qa-readonly-all.ps1` after route/layout/read-model changes. |
...
| R17 | Production release is treated as a continuation of local QA. | Critical | Medium | Open | Require release gate, staging signoff, rollback plan, tag approval, and explicit deploy approval. |
...
| R19 | Backend smoke testing accidentally uses production endpoints. | Critical | Low | Open | Use local/staging only; never hardcode production URLs; sanitize smoke instructions. |
...
- Validate both no-backend mock mode and deliberately configured backend read mode.
...
1. `R01` Mock fixtures drift from backend read-model truth.
2. `R02` Backend read integration breaks mock-first local development.
3. `R04` Production auth without a clear session/role model.
4. `R05` Token or secret leakage.
5. `R10` Write actions without idempotency, conflict handling, or audit.
