...
| R17 | Production release is treated as a continuation of local QA. | Critical | Medium | Watching | `FRONTEND_V2_INTERNAL_PILOT_READINESS.md` separates internal pilot readiness from production readiness; release checklist still requires staging signoff, rollback plan, tag approval, and explicit deploy approval. |
