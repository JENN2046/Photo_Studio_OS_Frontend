...
| Backend signoff guards | Production-like, credentialed, wrong-scope, non-HTTPS staging, and query-bearing backend URLs are rejected before smoke; expected 403 / 404 signoff modes are explicitly exposed. | `scripts\qa-backend-read-signoff-guards.ps1` | Local ready |
...
- Guarded local/staging backend signoff through `scripts\qa-backend-read-signoff.ps1`, including explicit expected failure-state runs for approved 403 / 404 fixtures.
