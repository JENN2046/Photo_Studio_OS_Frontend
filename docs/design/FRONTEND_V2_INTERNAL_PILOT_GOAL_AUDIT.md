...
| Keep role matrix complete. | `scripts\qa-auth-role-matrix.ps1` | Static QA checks 7 roles, 11 routes, 77 cells, session states, access labels, and auth rehearsal signals. | Covered locally |
...
| Keep auth/backend enforcement evidence concrete. | `docs\design\FRONTEND_V2_AUTH_BACKEND_ENFORCEMENT_SIGNOFF.md`, `scripts\qa-auth-backend-enforcement-signoff.ps1` | Static QA checks owner decisions, role mapping, endpoint enforcement, staging fixtures, no-secret boundaries, and display-only frontend gate language. | Covered locally |
...
| Preserve latest local validation evidence. | `docs\design\FRONTEND_V2_INTERNAL_PILOT_LOCAL_VALIDATION_LOG.md` | Latest local aggregate run is recorded as local-only evidence, with backend/auth/staging gaps still explicit. | Covered locally |
...
- `scripts\qa-internal-pilot-signoff-record.ps1`
