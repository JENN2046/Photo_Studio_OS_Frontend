...
| Release-boundary docs QA. | `scripts/qa-release-boundary-docs.ps1` checks internal-pilot, release, signoff, risk, and review docs keep release execution unapproved and block non-local non-example URLs. | Keep this green until explicit release/signoff approval is recorded through the proper release path. | Pilot QA done |
