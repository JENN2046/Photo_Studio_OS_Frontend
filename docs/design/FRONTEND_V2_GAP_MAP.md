...
- S2 backend read smoke now has `scripts/qa-backend-read-all.ps1`, which runs connected-path local mock backend smoke, local mock-backend 403 / 404 boundary smoke, and unreachable-backend failure smoke without editing `.env`.
...
| Backend read smoke aggregation. | `scripts/qa-backend-read-all.ps1` runs connected-path mock backend smoke, mock-backend 403 / 404 boundary smoke, and unreachable-backend failure smoke in sequence. | Use this local aggregate before requesting a real staging backend smoke. | S2 done |
| Backend connected-path smoke. | `scripts/qa-backend-read-smoke-mock.ps1` starts a temporary localhost read-model mock backend and verifies Command Center plus the four read-model pages through the real backend-read client path; the shared smoke harness confirms all five expected backend read paths were actually requested and can rehearse 403 / 404 responses through `-ResponseMode`. | Keep this wrapper GET/OPTIONS only; use it before any real staging backend smoke so connected UI regressions are caught locally. | S2 done |
...
| Real backend read-model smoke. | Local connected/failure smoke is automated; guarded signoff wrapper is available for ready and explicit expected 403 / 404 states; real backend smoke is still intentionally not run without a local or staging backend base URL. | Run `scripts\qa-backend-read-signoff.ps1` only with an approved local/staging backend URL outside this repo. | S2 blocked on backend URL |
...
28. Completed: S2 local backend connected, 403, 404, and failure smoke aggregation through `scripts/qa-backend-read-all.ps1`.
