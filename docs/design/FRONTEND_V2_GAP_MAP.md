...
| Role-derived access posture. | Runtime chips now show role-matrix-derived access as `完全`, `只读`, `摘要`, or `无权`; auth QA covers no-access, read-only partial, summary-only partial, and full role paths. | Keep this frontend display-only; backend must still enforce real authorization. | S3 done |
