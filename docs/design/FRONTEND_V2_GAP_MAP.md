...
| Auth / Role gates | Mock-first auth state machine with 7 roles, 10-route permission matrix, 6 gate components (SignedOut, Expired, Loading, Error, Forbidden, InsufficientRole), role-derived access labels, and `?authState=` / `?authRole=` DEV debug rehearsal. | `src/features/auth/authTypes.ts`, `src/features/auth/useAuthState.ts`, `src/features/auth/AuthGate.tsx`, `src/App.tsx`, `scripts/qa-readonly-auth-states.ps1` |
