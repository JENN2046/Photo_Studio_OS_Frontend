...
| Internal pilot QA aggregate. | `scripts/qa-internal-pilot-readiness.ps1` runs lint, build, local validation, backend aggregate smoke, live env role QA, auth-state QA, and full read-only browser QA in one local flow. | Use this before calling a local build an internal-pilot candidate; it still does not replace real backend/auth staging signoff. | Pilot QA done |
