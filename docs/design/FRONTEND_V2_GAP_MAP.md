...
- S2 backend read smoke now has `scripts/qa-backend-read-all.ps1`, which runs connected-path local mock backend smoke, local mock-backend 403 / 404 boundary smoke, local mock-backend empty / partial / stale data-state smoke, and unreachable-backend failure smoke without editing `.env`.
- S2 local/staging signoff now has `scripts/qa-backend-read-signoff.ps1`, a guarded wrapper that rejects production-like URLs, can run explicit expected 403 / 404 failure-state smoke or empty / partial / stale 200 data-state smoke, and reruns local gates after backend read smoke.
- S2 signoff guard QA now has `scripts/qa-backend-read-signoff-guards.ps1`, which checks unsafe backend URL rejection plus signoff failure-state and data-state option exposure before smoke can run.
...
- S3 role matrix integrity now has `scripts/qa-auth-role-matrix.ps1`, which checks 7 roles, 10 routes, 70 matrix cells, session states, access labels, and auth rehearsal signals.
- S3 auth provider preflight now has `scripts/qa-auth-provider-preflight.ps1`, which checks the required provider/session/role-claim decisions, staging fixtures, and backend enforcement blockers stay explicit before real auth work.
...
- Internal pilot readiness now has a single local aggregate, `scripts/qa-internal-pilot-readiness.ps1`, that chains local validation, backend read smoke, optional approved backend signoff with explicit expected data/failure state passthrough, live env role QA, and browser QA.
- Local validation now includes `scripts/qa-package-boundary.ps1`, which checks the approved Vite/React/TypeScript top-level dependency boundary and package-lock root metadata.
- Local validation now includes `scripts/qa-readonly-source-boundary.ps1`, a source-level guard against write methods, file inputs, signed URLs, browser storage, token handling, storage-provider URLs, and public-access enablement.
- Local validation now includes `scripts/qa-backend-read-contract-map.ps1`, which checks the five backend read fetchers, smoke routes, mock backend paths, and backend smoke plan stay synchronized.
- Local validation now includes `scripts/qa-internal-pilot-manifest.ps1`, which checks required source, QA, and documentation artifacts for the internal-pilot goal.
- Local validation now includes `scripts/qa-internal-pilot-readiness-guards.ps1`, which checks internal-pilot aggregate expected backend state options stay exposed and rejects mixed modes before long QA starts.
- Local validation now includes `scripts/qa-internal-pilot-signoff-record.ps1`, which checks the final signoff template covers the current evidence stack while staying unapproved.
- Local validation now includes `scripts/qa-internal-pilot-goal-audit.ps1`, which keeps the internal-pilot status at `LOCAL_FRONTEND_READY_CANDIDATE` until real backend/auth signoff blockers are cleared.
- Local validation now includes `scripts/qa-release-boundary-docs.ps1`, which keeps internal-pilot, signoff, release, risk, and review docs from silently implying production release approval.
- Local-only internal pilot aggregate evidence is recorded in `docs/design/FRONTEND_V2_INTERNAL_PILOT_LOCAL_VALIDATION_LOG.md`; it does not replace approved backend/auth/staging signoff.
...
| Runtime state surface | Command Center and the four read-model pages show source/status/transport/auth-session/auth-role/auth-access/write-boundary chips from their frontend runtime hooks through a shared renderer. | `src/components/panels/RuntimeChipList.tsx`, `src/features/command-center/useCommandCenterSnapshot.ts`, `src/features/command-center/CommandCenter.tsx`, `src/features/read-models/useBackendReadModel.ts`, `src/features/read-models/ReadModelPages.tsx` |
| Auth / Role gates | Mock-first auth state machine with 7 roles, 10-route permission matrix, 6 gate components (SignedOut, Expired, Loading, Error, Forbidden, InsufficientRole), role-derived access labels, and `?authState=` / `?authRole=` DEV debug rehearsal plus `VITE_BACKEND_USER_ROLE` env-role QA. | `src/features/auth/authTypes.ts`, `src/features/auth/useAuthState.ts`, `src/features/auth/AuthGate.tsx`, `src/App.tsx`, `scripts/qa-readonly-auth-states.ps1`, `scripts/qa-readonly-auth-live-roles.ps1` |
...
| Backend read smoke aggregation. | `scripts/qa-backend-read-all.ps1` runs connected-path mock backend smoke, mock-backend 403 / 404 boundary smoke, mock-backend empty / partial / stale data-state smoke, and unreachable-backend failure smoke in sequence. | Use this local aggregate before requesting a real staging backend smoke. | S2 done |
| Backend connected-path smoke. | `scripts/qa-backend-read-smoke-mock.ps1` starts a temporary localhost read-model mock backend and verifies Command Center plus the four read-model pages through the real backend-read client path; the shared smoke harness confirms all five expected backend read paths were actually requested and can rehearse 403 / 404 plus empty / partial / stale responses through `-ResponseMode`. | Keep this wrapper GET/OPTIONS only; use it before any real staging backend smoke so connected UI regressions are caught locally. | S2 done |
| Backend signoff guard QA. | `scripts/qa-backend-read-signoff-guards.ps1` verifies production-like, credentialed, wrong-scope, non-HTTPS staging, and query-bearing URLs are rejected before backend smoke can start, and that expected failure-state plus data-state signoff options stay exposed. | Keep unsafe URL rejection and expected 403 / 404 plus empty / partial / stale signoff options covered whenever `qa-backend-read-signoff.ps1` changes. | S2 done |
| Role-derived access posture. | Runtime chips now show role-matrix-derived access as `完全`, `只读`, `摘要`, or `无权`; auth QA covers no-access, read-only partial, summary-only partial, and full role paths. | Keep this frontend display-only; `summary-only` is a presentation rehearsal, and backend must still enforce real authorization. | S3 done |
| Auth role matrix static QA. | `scripts/qa-auth-role-matrix.ps1` checks the source role matrix for 7 roles, 10 routes, 70 cells, session states, access labels, and no token storage/header signals in auth source. | Keep expected matrix changes intentional and review docs whenever product role policy changes. | S3 done |
| Auth provider preflight QA. | `scripts/qa-auth-provider-preflight.ps1` checks real-auth prerequisites, staging fixtures, and backend authorization blockers remain documented before provider integration. | Keep this guard green until the backend/platform owner provides the actual auth provider and enforcement signoff. | S3 done |
| Auth/backend enforcement signoff pack. | `docs/design/FRONTEND_V2_AUTH_BACKEND_ENFORCEMENT_SIGNOFF.md` and `scripts/qa-auth-backend-enforcement-signoff.ps1` define the owner decisions, role mapping, endpoint enforcement, staging fixtures, and no-secret evidence boundaries needed before final S3 signoff. | Use this as the evidence intake checklist when backend/platform owners provide real auth and enforcement proof; do not use it to implement auth in the frontend. | S3 done |
| Live env role QA. | `scripts/qa-readonly-auth-live-roles.ps1` verifies representative non-debug role paths by setting `VITE_BACKEND_USER_ROLE` only in temporary child Vite processes. | Run with no existing dev server when env-role behavior changes; do not use it as real auth enforcement. | S3 done |
| Internal pilot QA aggregate. | `scripts/qa-internal-pilot-readiness.ps1` runs lint, build, local validation, backend aggregate smoke, optional approved backend signoff with explicit expected data/failure state passthrough, live env role QA, auth-state QA, and full read-only browser QA in one local flow. | Use this before calling a local build an internal-pilot candidate; it skips real backend signoff unless `-ApprovedBackendBaseUrl` is explicitly provided and still does not replace platform-auth staging signoff. | Pilot QA done |
| Package boundary QA. | `scripts/qa-package-boundary.ps1` checks package.json/package-lock root dependencies stay limited to the approved Vite/React/TypeScript stack and no UI/chart/state/CSS framework is added. | Keep this guard aligned with explicit dependency approvals; do not weaken it for convenience. | Pilot QA done |
| Read-only source boundary QA. | `scripts/qa-readonly-source-boundary.ps1` scans `src` for write methods, file inputs, signed URL fields, browser storage, token handling, storage-provider URLs, and public-access enablement; both validation helpers run it. | Keep future exceptions explicit and reviewed; do not weaken this for convenience. | Pilot QA done |
| Backend read contract-map QA. | `scripts/qa-backend-read-contract-map.ps1` checks all five backend read fetchers, browser smoke routes, mock backend paths, and backend smoke-plan docs stay aligned. | Run this whenever backend read-model paths or smoke fixtures change, before any approved local/staging signoff. | S2 done |
| Internal pilot evidence manifest QA. | `scripts/qa-internal-pilot-manifest.ps1` checks required source, QA, and documentation artifacts for the current internal-pilot evidence chain. | Keep this manifest aligned when adding or renaming pilot-critical docs/scripts. | Pilot QA done |
| Internal pilot readiness guard QA. | `scripts/qa-internal-pilot-readiness-guards.ps1` checks aggregate approved backend expected data/failure state options stay exposed and rejects mixed expected-state modes before long QA starts. | Keep this guard green whenever `qa-internal-pilot-readiness.ps1` changes. | Pilot QA done |
| Internal pilot signoff record QA. | `scripts/qa-internal-pilot-signoff-record.ps1` checks the signoff template covers local, backend, auth, and boundary evidence while remaining unapproved. | Keep this guard green whenever the pilot evidence stack changes. | Pilot QA done |
| Internal pilot goal audit QA. | `scripts/qa-internal-pilot-goal-audit.ps1` checks the goal audit, readiness checklist, signoff record, auth preflight, release checklist, and pilot guard scripts still present the frontend as a local candidate with backend/auth blockers. | Keep this guard aligned when the goal status moves from local candidate to signed-off pilot. | Pilot QA done |
| Release-boundary docs QA. | `scripts/qa-release-boundary-docs.ps1` checks internal-pilot, local validation log, release, signoff, risk, and review docs keep release execution unapproved and block non-local non-example URLs. | Keep this green until explicit release/signoff approval is recorded through the proper release path. | Pilot QA done |
| Internal pilot local validation log. | `docs/design/FRONTEND_V2_INTERNAL_PILOT_LOCAL_VALIDATION_LOG.md` records the latest local aggregate run as local-only evidence, with approved backend/auth/staging gaps still explicit. | Update only after a real local aggregate run; do not use this as final signoff. | Pilot QA done |
...
| Real backend read-model smoke. | Local connected/failure smoke is automated; guarded signoff wrapper is available for ready, explicit expected 403 / 404 states, and explicit expected empty / partial / stale 200 states. Real backend smoke is still intentionally not run without a local or staging backend base URL. | Run `scripts\qa-backend-read-signoff.ps1` only with an approved local/staging backend URL outside this repo. | S2 blocked on backend URL |
...
28. Completed: S2 local backend connected, 403, 404, empty, partial, stale, and failure smoke aggregation through `scripts/qa-backend-read-all.ps1`.
