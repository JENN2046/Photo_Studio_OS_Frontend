...
- S2 signoff guard QA now has `scripts/qa-backend-read-signoff-guards.ps1`, which checks unsafe backend URL rejection before smoke can run.
...
| Backend signoff guard QA. | `scripts/qa-backend-read-signoff-guards.ps1` verifies production-like, credentialed, wrong-scope, non-HTTPS staging, and query-bearing URLs are rejected before backend smoke can start. | Keep unsafe URL rejection covered whenever `qa-backend-read-signoff.ps1` changes. | S2 done |
