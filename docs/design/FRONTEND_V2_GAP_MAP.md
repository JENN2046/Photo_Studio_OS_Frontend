...
- S3 role matrix integrity now has `scripts/qa-auth-role-matrix.ps1`, which checks 7 roles, 11 routes, 77 matrix cells, session states, access labels, and auth rehearsal signals.
...
| Auth role matrix static QA. | `scripts/qa-auth-role-matrix.ps1` checks the source role matrix for 7 roles, 11 routes, 77 cells, session states, access labels, and no token storage/header signals in auth source. | Keep expected matrix changes intentional and review docs whenever product role policy changes. | S3 done |
