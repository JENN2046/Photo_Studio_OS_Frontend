...
- S2 local/staging signoff now has `scripts/qa-backend-read-signoff.ps1`, a guarded wrapper that rejects production-like URLs, can run explicit expected 403 / 404 failure-state smoke, and reruns local gates after backend read smoke.
- S2 signoff guard QA now has `scripts/qa-backend-read-signoff-guards.ps1`, which checks unsafe backend URL rejection and signoff failure-state option exposure before smoke can run.
...
| Backend signoff guard QA. | `scripts/qa-backend-read-signoff-guards.ps1` verifies production-like, credentialed, wrong-scope, non-HTTPS staging, and query-bearing URLs are rejected before backend smoke can start, and that expected failure-state signoff options stay exposed. | Keep unsafe URL rejection and expected 403 / 404 signoff options covered whenever `qa-backend-read-signoff.ps1` changes. | S2 done |
