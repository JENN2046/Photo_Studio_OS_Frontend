...
| Internal pilot goal audit QA. | `scripts/qa-internal-pilot-goal-audit.ps1` checks the goal audit, readiness checklist, signoff record, auth preflight, and release checklist still present the frontend as a local candidate with backend/auth blockers. | Keep this guard aligned when the goal status moves from local candidate to signed-off pilot. | Pilot QA done |
