...
function fixture(effectsEnabled=false,{fakeTimers=false}={}){
  const timers=new Map(),intervals=new Map();let timerId=0,intervalId=0;
...
function actualClient(){
  const load=url=>{const module={exports:{}};const output=ts.transpileModule(readFileSync(url,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText;new Function('require','module','exports',output)(name=>name.startsWith('.')?load(new URL(name+'.ts',url)):require(name),module,module.exports);return module.exports;};
  return load(new URL('../src/features/workbench/client.ts',import.meta.url));
}
test('actual control GET timeout exposes explicit retry and retains known safe-stop controls on later failures',async t=>{
  t.mock.timers.enable({apis:['setTimeout']});const ui=fixture(true);let hang=true,reads=0;const signals=[];
  ui.props.client=actualClient().createWorkbenchClient('','https://fixture.invalid','synthetic-session',true,async(target,options)=>{
    reads++;signals.push(options.signal);if(target.endsWith('/control')&&hang)return new Promise(()=>{});
    const data=target.endsWith('/control')?{mode:'RUNNING',revision:4,pool:{busy:true,phase:'RUNNING'}}:target.endsWith('/ledger')?{id:'grant-id',costUnit:'synthetic_credit',maxTotalCredits:1,reservedCredits:0,consumedCredits:0}:{id:'operation-id',state:'READY',attempts:[]};return {status:200,ok:true,json:async()=>({data})};
  });await ui.commit();assert.equal(reads,3);assert.equal(ui.button('提交控制变更'),undefined);
  t.mock.timers.tick(30000);await flush();await ui.commit();assert.ok(ui.button('重试读取执行详情'));assert.ok(signals[2].aborted);
  hang=false;await ui.click('重试读取执行详情');await ui.commit();assert.ok(ui.button('提交控制变更'));assert.equal(ui.button('重试读取执行详情'),undefined);
  hang=true;ui.props.refresh++;await ui.commit();const pendingReadCount=reads;for(const callback of ui.intervals.values())callback();await ui.commit();assert.equal(reads,pendingReadCount,'poll does not cancel a pending detail read');
  assert.equal(ui.button('提交控制变更').props.disabled,false);t.mock.timers.tick(30000);await flush();await ui.commit();assert.ok(ui.button('重试读取执行详情'));assert.equal(ui.button('提交控制变更').props.disabled,false);
  const mode=ui.nodes(ui.render(),n=>n.props?.label==='控制模式')[0].props.children;mode.props.onChange({target:{value:'RUNNING'}});assert.equal(ui.button('提交控制变更').props.disabled,true);
  hang=false;await ui.click('重试读取执行详情');await ui.commit();assert.equal(ui.button('重试读取执行详情'),undefined);assert.equal(ui.button('提交控制变更').props.disabled,false);
});
