...
function fixture(effectsEnabled=false){
  ...
  new Function('require','module','exports','window','document',code)(name=>name==='react'?hooks:name==='./parts'?{Field:'Field',Panel:'Panel',Id:'Id'}:name==='./domain'?{createIntentKey:()=>{throw Error('NO_WRITE');},formatCredits:String}:require(name),module,module.exports,{setInterval(){return 1;},clearInterval(){}},{hidden:false});
  const props={client:{get(path){if(effectsEnabled&&!/budget-entries|\/events\?/.test(path))return Promise.resolve(path.endsWith("/ledger")?seeded[4]:path.endsWith("/control")?{mode:"PAUSED",revision:1,pool:{busy:false}}:seeded[5]);const d=deferred();requests.push({...d,path});return d.promise;},post(){throw Error('NO_WRITE');}},prefix:'/projects/project-id',unitId:'unit-id',recipeId:'',localRecipes:[],grants:[],operations:[],disabled:false,controlDisabled:false,controlUncertain:false,run(){throw Error('NO_WRITE');},runControl(){throw Error('NO_WRITE');},refresh:0};
